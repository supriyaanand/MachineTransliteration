...
def language_features(token):
    ...
    func_dict = {'check_seq' :check_seq, 'check_ends_with_in' : check_ends_with_in, 'check_ends_with_n' : check_ends_with_n,
                 ...
                 'check_contains_in' : check_contains_in, 'check_is_smiley' : check_is_smiley, 'check_is_punct' : check_is_punct, 'check_is_num' : check_is_num}
    ...
    pat_dict = {'check_seq' : p_seq, 'check_ends_with_in' : p_ends_with_in, 'check_ends_with_n' : p_ends_with_n,
                 ...
                 'check_contains_in' : p_contains_in, 'check_is_smiley' : p_is_smiley, 'check_is_punct' : p_is_punct, 'check_is_num' : p_is_num}

    func_list = ['check_seq', 'check_ends_with_in', 'check_ends_with_n', 'check_ends_with_i','check_ends_with_o',
                 'check_contains_in', 'check_is_smiley', 'check_is_punct', 'check_is_num']
    count = 1
...
if __name__ == "__main__":
    ...
    for line in u:
        all_tokens.extend(token[0] for token in line)
    ...
    for line in u:
        train_featuresets.extend([(language_features(token[0]), token[1]) for token in line])
    #print featuresets
    ...
    random.shuffle(train_featuresets)
    train,tests = train_featuresets[:26000],train_featuresets[26000:]
    classifier = nltk.NaiveBayesClassifier.train(train)
    print(nltk.classify.accuracy(classifier, tests))
    print classifier.show_most_informative_features(15)

    errors = []
    for tok,tag in test_tags:
        guess = classifier.classify(language_features(tok.lower()))
        if guess != tag:
    ...
    for line in testdata:
        ...
        #print words
        for token in words:
            tag = classifier.classify(language_features(token))
            line_op = line_op + token + '\\' + tag + ' '
        line_op = line_op + '\n'
        FW.write(line_op.encode('utf-8'))
    FH.close()
    FW.close()
